...
z2 = W1 * data + repmat(b1,1,10000);
a2 = arrayfun(@sigmoid,z2);

rho = sum(a2,2)/10000;
rho_cost = beta * sum(arrayfun(@sparsity,rho));

z3 = W2 * a2 + repmat(b2,1,10000);
a3 = arrayfun(@sigmoid,z3);

% Cost

cost = .5 * sum(sum((a3 - data).^2))/ 10000;
...
% Output Layer
delta_3 = -(data - a3) .* arrayfun(@sigderiv,z3);

% Hidden Layer
delta_2 = ((transpose(W2) * delta_3) + repmat(beta * arrayfun(@sparsity_deriv,rho),1,10000)) .* arrayfun(@sigderiv,z2);

% partial derivatives
W1grad = delta_2 * transpose(data)/10000+ lambda * W1; 
W2grad = delta_3 * transpose(a2)/10000+ lambda * W2 ;
b1grad = sum(delta_2,2)/10000; 
b2grad = sum(delta_3,2)/10000;
